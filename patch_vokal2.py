...
js = re.sub(r'function bukaSenaraiHuruf\(jenis\) \{[\s\S]*?paparSkrin\([^)]+\);\n?\}', new_bukaSenarai, js)

# Update bukaModalPilihJenisHuruf again in case it didn't apply
...
js = re.sub(r'function bukaModalPilihJenisHuruf\(id\) \{[\s\S]*?\}\n\nfunction bukaSenaraiHuruf', new_bukaModal + "\n\nfunction bukaSenaraiHuruf", js)
